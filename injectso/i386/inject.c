...
const char *libc_path = "/lib32/libc-2.27.so";  
const char *linker_path = "/lib32/libc-2.27.so";  
...
{  
    ...
    printf("base addr :  %x , module_name :  %s\n" ,remote_handle,module_name);
    return ret_addr;  
}  
...
{  
    ...
    char buf[200] = {0};
    ptrace_readdata(target_pid , map_base , buf , strlen(library_path)+1);
    printf("input str %s \n" , buf);
